	...
	uint32_t sof_event_status,uint64_t delta_ts)
{
	...
	CAM_DBG(CAM_ISP,"req id:%lld frame num:%lld bt_ts:0x%llx pre_bt_ts:0x%llx diff:0x%llx",request_id, ctx_isp->frame_id,ctx_isp->boot_timestamp, ctx_isp->prev_boot_timestamp,delta_ts);
	...
	uint32_t sof_event_status)
{
	...
	if (cam_req_mgr_notify_message(&req_msg,
		...
		CAM_ERR(CAM_ISP,
			...
			request_id);
	delta_ts = ctx_isp->sof_timestamp_val - ctx_isp->prev_sof_timestamp_val;

	__cam_isp_ctx_send_sof_boot_timestamp(ctx_isp,request_id, sof_event_status,(ctx_isp->prev_sof_timestamp_val == 0) ?ctx_isp->boot_timestamp :delta_ts);
	ctx_isp->prev_sof_timestamp_val = ctx_isp->sof_timestamp_val;
